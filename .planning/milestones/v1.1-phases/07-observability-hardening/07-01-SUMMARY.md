...
provides:
  ...
  - "Sentry.init environment field uses truthy fallback VITE_NETLIFY_CONTEXT || MODE so deploy-preview events report environment='deploy-preview' — `||` (not `??`) because Netlify's shell substitution of an unset $CONTEXT yields the empty string, which `??` would forward verbatim as `environment: \"\"` (WR-03 hotfix `c9475b8`) (Round-2 HIGH-1)"
...
key-files:
  created: []
  modified:
    - "src/main.tsx — createRoot now passes options object with all three React 19 error hooks each wired through the taggedHandler factory (uncaught passes undefined to Sentry.reactErrorHandler so mechanism.handled stays false; caught/recoverable pass a no-op `() => {}` so mechanism.handled flips to true, matching React's own classification — WR-01 hotfix `5960565`); Sentry.ErrorBoundary now carries beforeCapture + onError props; Sentry.init environment field uses truthy fallback VITE_NETLIFY_CONTEXT || MODE (WR-03 hotfix `c9475b8` — empty-string shell substitution of unset $CONTEXT falls through to MODE); ErrorInfo type imported from 'react'"
...
key-decisions:
  ...
  - "Sentry.init environment now uses truthy fallback VITE_NETLIFY_CONTEXT || MODE (Round-2 HIGH-1, intentional in-scope addition; final form locked in WR-03 `c9475b8`): deploy-preview events no longer mislabel as environment='production'. `||` (not `??`) is the hardened form because Netlify's shell substitution of an unset $CONTEXT produces the empty string `VITE_NETLIFY_CONTEXT=`, which `??` would forward verbatim as `environment: \"\"` — `||` falls through to MODE on empty-string AND undefined"
...
patterns-established:
  ...
  - "Sentry.init environment truthy-fallback honors deploy context (VITE_NETLIFY_CONTEXT || MODE) so triage can filter by environment=deploy-preview separately from environment=production — `||` chosen over `??` because Netlify shell-substitutes an unset $CONTEXT to the empty string, which only truthy fallback collapses to MODE"
...
**React 19 createRoot tagged-handler factories route render-phase errors into Sentry with dedupe-resilient boundary tagging (uncaught passes `undefined` to preserve `mechanism.handled=false`; caught/recoverable pass a no-op callback to flip it to `true`, matching React's classification), plus Rolldown keepNames preserves source identifiers (RootLayout, AppErrorFallback, ConsentChip, etc.) through Oxc mangling, plus VITE_NETLIFY_CONTEXT shell-export feeds Sentry.init `environment` via truthy `||` fallback (not `??`, because empty-string shell substitution must fall through to MODE) so deploy-preview events stop mislabeling as production.**
...
- Sentry.init environment field now uses truthy fallback VITE_NETLIFY_CONTEXT || MODE (Round-2 HIGH-1; final form locked in WR-03 `c9475b8`) — deploy-preview events report environment='deploy-preview' instead of misleading 'production'. `||` over `??` because an unset Netlify $CONTEXT shell-substitutes to the empty string `VITE_NETLIFY_CONTEXT=`, which `??` would forward verbatim as `environment: ""`
...
- `src/main.tsx` — createRoot now passes a container variable + options object with onUncaughtError/onCaughtError/onRecoverableError each wired through `taggedHandler(kind)` factory that sets `boundary='app-root'` and `react.errorHandlerKind=<kind>` on the active scope before delegating to `Sentry.reactErrorHandler`. The factory selects the inner callback per kind: `const cb = kind === 'uncaught' ? undefined : () => {}` — `uncaught` passes `undefined` so Sentry's SDK reports `mechanism.handled=false` (preserves React's crash-free-sessions metric), while `caught`/`recoverable` pass a no-op `() => {}` so the SDK flips `mechanism.handled=true`, matching React's own classification of these paths as handled (WR-01 hotfix `5960565`). Sentry.ErrorBoundary now carries `beforeCapture={(scope) => scope.setTag('boundary','app-root')}` AND an `onError` belt calling `Sentry.captureException(error, { tags: { boundary: 'app-root' }, contexts: { react: { componentStack }, linked_event: { eventId } } })`. `Sentry.init` `environment` field is now `import.meta.env.VITE_NETLIFY_CONTEXT || import.meta.env.MODE` — `||` (not `??`) because Netlify's shell substitution of an unset `$CONTEXT` yields the empty string, which only truthy fallback collapses to `MODE` (WR-03 hotfix `c9475b8`). `ErrorInfo` type imported from `'react'` (verified against current shipped source) per Sentry's canonical TS guidance (Round-3 LOW-3 prose-only correction; verbatimModuleSyntax requires the explicit `type` keyword).
...
- **Tagged-handler factory closure over `kind`** — preferred a single factory that closes over `'uncaught' | 'caught' | 'recoverable'` over three near-identical handler functions because (a) it keeps the boundary-tag setting in one place (DRY across the three React 19 hooks), (b) preserves the per-kind `react.errorHandlerKind` tag for triage, (c) allows the dev-only `console.warn` to fire only on the `uncaught` kind without conditional branching at every call site, AND (d) lets the factory select the inner Sentry callback per kind — `undefined` for `uncaught` so the SDK records `mechanism.handled=false` (Sentry SDK `error.js:97-99`: `mechanism.handled = !!callback`), and a no-op `() => {}` for `caught`/`recoverable` so `mechanism.handled=true` matches React's own classification of those paths as handled (WR-01 hotfix `5960565`). This preserves the crash-free-sessions metric for genuine uncaught crashes while keeping caught/recoverable out of that metric.
...
- **Issue:** The acceptance criterion expected the bare `import.meta.env.MODE,` substring to be absent. The new fallback expression `VITE_NETLIFY_CONTEXT || import.meta.env.MODE,` (final form per WR-03; initially `??`, hardened to `||` to handle Netlify empty-string shell substitution) legitimately contains that substring as the right-hand operand. The criterion's pattern was overly literal.
- **Substantive intent:** "no bare unguarded `environment: import.meta.env.MODE,` line" — satisfied. The line now reads `environment: import.meta.env.VITE_NETLIFY_CONTEXT || import.meta.env.MODE,` which is the Round-2 HIGH-1 fix as hardened by WR-03.
