...
| 05-03/T2 | 03 | 1 | INFR-02, TEST-06 | D-13 | T-05-05, T-05-06, T-05-07 | unit + shell-grep + bundle budget | `grep -q 'Sentry.init' src/main.tsx && test -z "$(grep defaultPreload src/main.tsx)" && test -z "$(grep replayIntegration src/main.tsx)" && grep -q 'posthog.identify' src/contexts/AuthContext.tsx && test -z "$(grep -E 'posthog\.identify.*(email\|username\|discriminator)' src/contexts/AuthContext.tsx)" && grep -q loadSentryReplayIfConsented src/components/ConsentChip.tsx && grep -q analytics_opted_out src/components/ConsentChip.tsx && npm test -- --run && npm run build && BUNDLE_SIZE=$(du -ck dist/assets/*.js \| tail -1 \| awk '{print $1}') && [ $BUNDLE_SIZE -le 400 ]` (HIGH #1 no defaultPreload, M1 consent-gated replay, M3 bundle budget) | ✅ | ⬜ pending |
...
| 05-07/T2 | 07 | 2 | INFR-02 | D-01, D-02, D-03 | T-05-11, T-05-13 | shell-grep + yaml-parse | `grep -q "0 3 \* \* \*" .github/workflows/cron-sweep.yml && test -z "$(grep -E 'cron:.*0 0 \* \* \*' .github/workflows/cron-sweep.yml)" && grep -q X-Cron-Secret .github/workflows/cron-sweep.yml && grep -q 'Authorization: Bearer' .github/workflows/cron-sweep.yml && grep -q SUPABASE_ANON_KEY .github/workflows/cron-sweep.yml && grep -q '"success":true' .github/workflows/cron-sweep.yml && grep -q '"swept"' .github/workflows/cron-sweep.yml && grep -q '\-X POST' .github/workflows/cron-sweep.yml && test -z "$(grep continue-on-error .github/workflows/cron-sweep.yml)"` (HIGH #3 — Bearer header + body validation) | ✅ | ⬜ pending |
...
| 05-08/T3 | 08 | 3 | D-12 | D-12 | T-05-02 | manual (checkpoint) | Secrets visible in Netlify, Supabase, GH dashboards — GH list now includes SUPABASE_ANON_KEY (HIGH #3); LOCAL_* keys NO LONGER stored (M5) | n/a | ⬜ pending |
