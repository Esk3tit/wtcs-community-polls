...
- Changes to `Sentry.init` config, `sentryVitePlugin` config, sourcemap upload pipeline, or release-SHA wiring — all already correct from Phase 6.
...
- **D-08:** Evidence required (all four MUST land in `07-VERIFICATION.md` before the phase is called done):
  ...
  4. Built `dist/assets/*.js.map` `names[]` excerpt + `grep '__name(' dist/assets/*.js | head` output, proving `keepNames: true` mechanically took effect during the build (roadmap success criterion #3 — catches misconfig that Sentry's UI alone might mask).
...
### Files to modify
- `src/main.tsx` — wire React 19 root error hooks; add `onError` belt to `Sentry.ErrorBoundary`. (Sentry.init block stays untouched.)
...
- `src/main.tsx` `Sentry.init({...})` block — already correct (DSN, environment, release SHA, browserTracingIntegration, traces/replays sample rates). DO NOT touch.
