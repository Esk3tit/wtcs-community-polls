...
|-----------------|------|-------------|-----------------|-----------|-------------------|-------------|--------|
| 16-01 (PERF-01) | 1 | PERF-01 | `vite.config.ts` throws at config-load when `ANALYZE && NETLIFY_CONTEXT=production` (prevents OBSV-04 sourcemap-upload regression) | manual / config-load assertion | `NETLIFY_CONTEXT=production ANALYZE=true npm run build` (expect non-zero exit + error citing OBSV-04) | manual-only | ⬜ pending |
...
|----------|-------------|------------|-------------------|
| `vite.config.ts` throws at config-load on production+ANALYZE collision | PERF-01 (D-09) | Config-load assertion runs before any test framework; exercised by running the build with the trap env-vars set | `NETLIFY_CONTEXT=production ANALYZE=true npm run build` — expect non-zero exit and error message naming OBSV-04 |
