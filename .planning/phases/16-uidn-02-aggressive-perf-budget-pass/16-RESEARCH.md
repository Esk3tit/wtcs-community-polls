...
> **AUTHORITATIVE-SOURCE NOTE (folds cross-AI review cycle 2).** Where this research and the corresponding `16-NN-PLAN.md` disagree, **the PLAN is authoritative.** Three design refinements were discovered during planning and verification that supersede prose below — defer to the PLANs:
> 1. **PostHog Suspense pattern is SIBLING-loader, NOT child-wrapping.** `PostHogGate` ALWAYS renders `{children}` directly and mounts a null-rendering side-effect loader as a SIBLING inside `<Suspense fallback={null}>`. `PostHogProviderInner` renders `null` — it is NOT a `<PostHogProvider client={posthog}>{children}</PostHogProvider>` wrapper. The original "wrap children in a lazy `<PostHogProvider>`" design (and the claim that Suspense "keeps children rendered through the dynamic-import window") was WRONG: Suspense replaces the entire suspending subtree with the `null` fallback, so wrapping children blanks/remounts the router. Authoritative source: `16-03-PLAN.md`.
> 2. **Facade-only analytics client.** The consumer audit (`grep -rn "usePostHog" src/`) confirms NO component consumes `usePostHog()`, so there is NO React context provider in the tree once consent resolves. D-01/D-02 wording about "`PostHogProvider` available to the tree" is superseded by the facade-only design. Authoritative source: `16-03-PLAN.md`.
> 3. **`vendor-react` is the React runtime family (`react` + `react-dom` + `scheduler`) via the FUNCTION-form matcher as primary.** D-04's "react and react-dom only" undercounts — `scheduler` (React's own runtime dep) belongs in the same cache-stable chunk. Authoritative source: `16-04-PLAN.md`.
...
### Locked Decisions
- **D-01 — Lazy PostHogGate gate (SIBLING-loader, FINAL design).** `PostHogGate` ALWAYS renders `{children}` directly (unconditionally), and mounts a null-rendering side-effect loader as a SIBLING inside `<Suspense fallback={null}>` ONLY when `ConsentContext.state === 'allow'`. Use `React.lazy(() => import(...))` to defer `posthog-js`. The loader (`PostHogProviderInner`) renders `null` — it is NOT a child-wrapping `<PostHogProvider>{children}</PostHogProvider>`. The sibling shape is load-bearing: it keeps the router subtree out of the suspending boundary so the dynamic-import window cannot blank/remount the router. When state is `'undecided'` or `'decline'`, the loader is simply not rendered and `posthog-js` is never fetched. Follow the existing `lazy(() => import('@/components/debug/RenderThrowSmoke'))` and `lazy(() => import('@/components/debug/DebugAuthOverlay'))` shape from `src/routes/__root.tsx:16` and `src/routes/[__smoke].tsx:13-17`.
...
- **D-04 — Two named chunks only.** `vendor-react` (covers the React runtime family — `react` + `react-dom` + `scheduler`, React's own runtime dep; TanStack Router stays in Rolldown auto-split) and `vendor-posthog` (covers `posthog-js`, including the `posthog-js/react` sub-path). Existing auto-split chunks for `supabase-js` and `sentry-replay` are left as auto-split.
...
- **D-09 — Plugin-order safety throw.** `vite.config.ts` throws an explicit `Error` at config-load time if `process.env.ANALYZE === 'true' && (process.env.CONTEXT === 'production' || process.env.NETLIFY_CONTEXT === 'production')`. CORRECTED (cycle 2): guard `CONTEXT` (Netlify-native, verified netlify.toml:9) AND legacy `NETLIFY_CONTEXT`; keying only on `NETLIFY_CONTEXT` makes the trap inert in real Netlify CI. Error message names OBSV-04 + Phase 15 sourcemap-upload chain explicitly. The build dies before producing any artifacts.
...
| `package.json` (edit) | New `build:analyze` script: `ANALYZE=true npm run build` (NOT the inline `ANALYZE=true tsr generate && tsc -b && vite build` — that scopes `ANALYZE` only to `tsr generate`, so `vite build` never sees the flag); new devDep `rollup-plugin-visualizer@7.0.1` (`--save-exact`) |
...
**What:** A consent gate that ALWAYS renders children, and — only when a precondition is met — ALSO mounts a lazy-loaded SIDE-EFFECT loader as a SIBLING of children. (CORRECTED from the original "wrap children in a lazy provider" framing — see the AUTHORITATIVE-SOURCE NOTE at the top of this file.)
**When to use:** When the loaded module is expensive AND optional based on runtime state (here: GDPR consent='allow'), and the children must stay continuously mounted.
**Existing in-repo analog:** `src/routes/__root.tsx:43-45` already uses `<Suspense fallback={null}><DebugAuthOverlay /></Suspense>` as the lazy-mount idiom — the lazy thing is mounted as its OWN element, NOT as a wrapper around the rest of the tree. The Phase 16 `<PostHogGate>` follows that exact shape: the lazy loader is a self-contained sibling, children pass through unconditionally.

**Shape (authoritative source: `16-03-PLAN.md`):**
1. `const LazyPostHogLoader = lazy(() => import('@/components/PostHogProviderInner').then(m => ({ default: m.PostHogProviderInner })))` — the inner module is a SIDE-EFFECT loader that calls `initPostHog()` + `posthog.setClient(client)` at module scope and renders `null` (it does NOT wrap children, does NOT import `posthog-js/react`).
2. The `<PostHogGate>` component reads consent via `useConsent()` and returns `<>{children}{state === 'allow' && <Suspense fallback={null}><LazyPostHogLoader /></Suspense>}</>`. `{children}` is ALWAYS rendered, as a SIBLING of the `<Suspense>` boundary — never nested inside it.
3. The Suspense `fallback={null}` is non-negotiable per UI-SPEC Loading & Suspense States. CORRECTION: the reason there is no spinner/blank is NOT that "Suspense keeps children rendered" (it does NOT — Suspense replaces its suspending subtree with the fallback). It is that `{children}` lives OUTSIDE the Suspense boundary as a sibling, and the only thing inside the boundary (`LazyPostHogLoader`) renders `null` anyway — so the import window is visually invisible.
...
**How to avoid:** D-09's throw-at-config-load. If `ANALYZE === 'true' && (CONTEXT === 'production' || NETLIFY_CONTEXT === 'production')`, throw before any artifact is produced (guard the Netlify-native `CONTEXT` var AND legacy `NETLIFY_CONTEXT` — keying only on the legacy name makes the trap inert in real Netlify CI). The throw message names OBSV-04 by name so a future maintainer reading the error log immediately understands the trap.
...
File `src/components/PostHogGate.tsx` (CORRECTED — authoritative source `16-03-PLAN.md`). Imports: `import { lazy, Suspense, type ReactNode } from 'react'`, `import { useConsent } from '@/hooks/useConsent'`. Declares `const LazyPostHogLoader = lazy(() => import('@/components/PostHogProviderInner').then(m => ({ default: m.PostHogProviderInner })))`. Component `function PostHogGate({ children }: { children: ReactNode })`: reads `state` from `useConsent()`; ALWAYS returns `<>{children}{state === 'allow' && <Suspense fallback={null}><LazyPostHogLoader /></Suspense>}</>` — `{children}` is rendered unconditionally as a SIBLING of the `<Suspense>` boundary, NEVER nested inside it. Inner module `src/components/PostHogProviderInner.tsx` (NAMED export, consumed by React.lazy via the `.then(m => ({ default: m.PostHogProviderInner }))` destructure): statically imports `initPostHog` from `@/lib/posthog` and `posthog` from `@/lib/posthog-facade`, calls `initPostHog()` once at module scope, calls `posthog.setClient(client)` from the facade, and the component body returns `null` (side-effect-only loader — it does NOT import `posthog-js/react`, does NOT render `<PostHogProvider>`, does NOT take children, because no component consumes `usePostHog()`).
...
A single `<action>` block at the start of the phase's first plan (CORRECTED — authoritative source `16-01-PLAN.md`): (a) `npm install --save-dev --save-exact rollup-plugin-visualizer@7.0.1` (gate behind `checkpoint:human-verify` per slopcheck protocol; `--save-exact` is mandatory to pin the literal `"7.0.1"`), (b) edit `vite.config.ts` to add the import, the env-gated mutual-exclusion block (visualizer OR sentryVitePlugin, never both), the D-09 throw at config-load when `process.env.ANALYZE === 'true' && (process.env.CONTEXT === 'production' || process.env.NETLIFY_CONTEXT === 'production')` — CRITICAL: guard `CONTEXT` (the var Netlify natively sets, verified netlify.toml:9) AND legacy `NETLIFY_CONTEXT`; keying only on `NETLIFY_CONTEXT` makes the trap INERT in real Netlify CI — and add a `build:analyze` npm script `"build:analyze": "ANALYZE=true npm run build"` (NOT the inline `ANALYZE=true tsr generate && tsc -b && vite build`, which scopes `ANALYZE` only to `tsr generate` so `vite build` never sees the flag). The visualizer plugin config: `{ filename: 'dist/stats.html', template: 'treemap', gzipSize: true, brotliSize: true, open: false }`. Verification: `ANALYZE=true npm run build` produces `dist/stats.html` AND the `vendor-react`/`vendor-posthog` names are NOT yet present (PERF-04 lands later); `CONTEXT=production ANALYZE=true npm run build` and `NETLIFY_CONTEXT=production ANALYZE=true npm run build` each exit non-zero with a clear error mentioning OBSV-04 — these are EXPECTED-FAILURE commands, so capture status+output separately (`out=$(...); status=$?`) and assert status≠0 AND output contains OBSV-04; do NOT pipe them into `grep` under `pipefail`/`-e` (that inverts the check into a false negative). Run the Phase 15 sourcemap-names guard `npm run build && node scripts/verify-sourcemap-names.mjs` to confirm the non-ANALYZE production build still produces all 7 allowlisted names.
...
Three coordinated `<action>` blocks (CORRECTED — sibling-loader, facade-only; authoritative source `16-03-PLAN.md`): (1) Create `src/lib/posthog-facade.ts` exporting a `posthog` namespace object with `identify(id)`, `reset()`, `opt_in_capturing()`, `opt_out_capturing()` — each pushes a thunk onto a module-scope queue (bounded by a documented cap, e.g. 50) if the internal client is null, else forwards directly; expose a `setClient(c)` that assigns the client and drains the queue once. (2) Create `src/components/PostHogProviderInner.tsx` (NAMED export, consumed by React.lazy via the `.then(m => ({ default: m.PostHogProviderInner }))` destructure) that statically imports `initPostHog` from `@/lib/posthog` (which transitively pulls `posthog-js` into the lazy chunk) and `posthog` from `@/lib/posthog-facade`, calls `initPostHog()` once at module-scope, calls `posthog.setClient(client)` from the facade, and the component body returns `null`. It does NOT import `posthog-js/react`, does NOT render `<PostHogProvider>`, and does NOT take children — it is a side-effect-only loader, because the consumer audit confirms NO component uses `usePostHog()` (facade-only analytics client). Create `src/components/PostHogGate.tsx` (named export) reading `useConsent()`; ALWAYS returns `<>{children}{state === 'allow' && <Suspense fallback={null}><LazyPostHogLoader /></Suspense>}</>` where `LazyPostHogLoader = lazy(() => import('@/components/PostHogProviderInner').then(m => ({ default: m.PostHogProviderInner })))`. CRITICAL: `{children}` is rendered unconditionally as a SIBLING of the `<Suspense>` boundary, NEVER nested inside the suspending loader — nesting children is the verified HIGH bug (blanks/remounts the router during the lazy-import window). (3) Edit `src/main.tsx` to swap `<PostHogProvider client={posthog}>...</PostHogProvider>` for `<ConsentProvider><PostHogGate>...</PostHogGate></ConsentProvider>` (inverting current order — ConsentProvider goes OUTER; `<RouterProvider>` is passed as PostHogGate's `children` and renders unconditionally as a sibling of the lazy loader). Edit `src/contexts/AuthContext.tsx` to change the `posthog` import from `@/lib/posthog` to `@/lib/posthog-facade` (call sites at lines 168, 181 stay identical). Edit `src/contexts/ConsentContext.tsx` similarly (call sites at lines 55, 58 stay identical). Remove the unused top-level `initPostHog()` call from `src/main.tsx:40` (it's now called inside `PostHogProviderInner.tsx`). Add unit tests at `src/__tests__/lib/posthog-facade.test.ts` (queue-then-flush + queue cap) and `src/__tests__/components/PostHogGate.test.tsx` (always-render-children + synchronous-children regression guard + conditional sibling-loader mount via mocked `useConsent`). Add the MANDATORY blocking Playwright gate `e2e/tests/posthog-consent-gate.spec.ts` (zero `/posthog/i` network before Allow against the production preview build). Verification: `npm run build` produces no errors; the mandatory Playwright gate passes; facade + component unit tests green.

### PERF-04 — Planner Instruction
A single `<action>` block (CORRECTED — function form is PRIMARY; authoritative source `16-04-PLAN.md`): extend `vite.config.ts` `build.rolldownOptions.output` with the boundary-anchored FUNCTION form `manualChunks: (id) => { if (/[\\/]node_modules[\\/](react|react-dom|scheduler)[\\/]/.test(id)) return 'vendor-react'; if (/[\\/]node_modules[\\/]posthog-js[\\/]/.test(id)) return 'vendor-posthog'; }`. The function form is primary because Rolldown's object-keyed resolution is unverified in this repo, and the boundary-anchored regex gives explicit, auditable control over exactly which `node_modules/<pkg>/` paths land in each chunk. The `react|react-dom|scheduler` alternation deliberately INCLUDES `scheduler` (React's own internal runtime dep) so the React runtime family stays in one cache-stable chunk. The boundary anchor (NOT `id.includes('react')`) is what excludes `@tanstack/react-router`, `@radix-ui/react-*`, `@sentry/react`, `react-error-boundary` (Pitfall 2). The OBJECT form `{ 'vendor-react': ['react', 'react-dom'], 'vendor-posthog': ['posthog-js', 'posthog-js/react'] }` is a FALLBACK only — use it only if the function form proves problematic AND it is then empirically verified against the treemap in this repo. Re-run `ANALYZE=true npm run build`; confirm `vendor-react-*.js` and `vendor-posthog-*.js` chunks emit; inspect the treemap to confirm `vendor-react` holds the React runtime family (react + react-dom + scheduler) and NOT TanStack Router (per Pitfall 2). Re-run the sourcemap-names guard. Verification: `ls dist/assets/ | grep -E '^vendor-(react|posthog)-'` returns two non-empty matches.
