...
- **D-01:** Lazy `<PostHogGate>` mounted only when `ConsentContext.state === 'allow'`. The gate uses `React.lazy(() => import(...))` to defer `posthog-js` out of the critical-path chunk. The gate ALWAYS renders `{children}` directly and — only when `state === 'allow'` — ALSO mounts a lazy SIDE-EFFECT loader as a SIBLING of children. Users who never accept never pay the posthog-js bundle cost. Matches the existing `lazy(() => import(...))` pattern in `src/routes/__root.tsx`. **REFINED during planning (folds cross-AI review cycle 2):** the design is a FACADE-ONLY analytics client — the consumer audit (`grep -rn "usePostHog" src/`) confirms NO component consumes `usePostHog()`, so there is NO React context provider in the tree. The lazy loader (`PostHogProviderInner`) renders `null` and does NOT import `posthog-js/react`. The earlier "lazy PostHogProvider wrapping children" framing was a verified HIGH bug (Suspense blanks/remounts the router when children are nested inside the suspending boundary); the corrected design keeps `{children}` as a sibling of `<Suspense>`. Authoritative source: `16-03-PLAN.md`.
- **D-02:** Thin facade module `src/lib/posthog-facade.ts` is statically imported by `AuthContext` + `ConsentContext` (and any future caller of `posthog.identify` / `posthog.reset` / `opt_in_capturing` / `opt_out_capturing`). The facade exposes a synchronous namespace-object API. Internally it queues calls (bounded by a documented cap) if the real client has not loaded yet and replays them after the dynamic import resolves via `setClient`; if already loaded, it forwards immediately. Keeps the context providers synchronous and preserves the current product semantics (identify is safe to call even when capturing is opt-out). **REFINED:** the facade IS the analytics client surface the app uses — there is no separate React context provider once consent resolves (facade-only client). Authoritative source: `16-03-PLAN.md`.
...
- **D-04:** Stay minimal per ROADMAP. Only two explicitly-named chunks: `vendor-react` (the React RUNTIME FAMILY — `react` + `react-dom` + `scheduler`, React's own internal runtime dep — and explicitly NOT `@tanstack/react-router`, which stays in Rolldown auto-split) and `vendor-posthog` (covers `posthog-js` and its `posthog-js/react` sub-export). Existing auto-split chunks for `supabase-js` (~213 KB) and `sentry-replay` (~140 KB) are left as auto-split — Rolldown is handling them correctly today and explicit naming risks fighting the bundler. Re-evaluate in a future phase only if cache-busting becomes a real problem. **REFINED during planning (folds cross-AI review cycle 2):** the matcher uses the boundary-anchored FUNCTION form as primary (`/[\\/]node_modules[\\/](react|react-dom|scheduler)[\\/]/`); the original "react and react-dom only" undercounted `scheduler`. Authoritative source: `16-04-PLAN.md`.
...
- **D-09:** `vite.config.ts` throws an explicit `Error` at config-load time if `process.env.ANALYZE === 'true' && (process.env.CONTEXT === 'production' || process.env.NETLIFY_CONTEXT === 'production')`. Error message names the trap directly (e.g., `Refusing production build with ANALYZE=true — sourcemap upload would be skipped, silently breaking the OBSV-04 evidence chain shipped in Phase 15`). The build dies before producing any artifacts, so an accidentally-toggled `ANALYZE` env var in Netlify can never silently ship a release with no sourcemap upload. **REFINED during planning (folds cross-AI review cycle 2):** the guard keys on `CONTEXT` (the var Netlify NATIVELY sets to `production` in a production deploy — verified `netlify.toml:9`) AND legacy `NETLIFY_CONTEXT`. Keying only on `NETLIFY_CONTEXT` (the original decision text) would make the trap INERT in real Netlify CI because Netlify never sets a bare `NETLIFY_CONTEXT`. Covers both Netlify CI (`CONTEXT`) and any local `NETLIFY_CONTEXT=production npm run build:analyze` invocation. Authoritative source: `16-01-PLAN.md`.
...
- `vite.config.ts` plugins array — env-gate inserts visualizer or sentryVitePlugin (mutually exclusive); throws if `ANALYZE && (CONTEXT === 'production' || NETLIFY_CONTEXT === 'production')` per D-09 (guard Netlify-native `CONTEXT` + legacy `NETLIFY_CONTEXT`).
