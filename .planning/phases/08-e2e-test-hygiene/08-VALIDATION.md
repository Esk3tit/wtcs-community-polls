...
| TEST-09-cleanup | Fixture inserts +1 then deletes -1 row per test (no leak) | integration | `psql "$DB_URL" -c "select count(*) from polls where title like '[E2E]%' and created_at > now() - interval '5 minutes'"` (expect 0 after suite) | ❌ Wave 0 — fixture file new |
