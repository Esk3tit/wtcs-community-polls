...
- **sha256 dupe-check** — after the screenshot run, group all 42 PNGs by sha256; if a collision group is NOT the per-width `bp-{w}-home.png` ↔ `bp-{w}-admin.png` whitelist pair (D-19 — intentional collision because unauth `/admin` redirects to `/` via AdminGuard), log the offending paths and exit non-zero. Catches the Phase 9 defect signature (4-of-30 byte-identical loading shells) deterministically. MANIFEST.json already records sha256 — implementation is ~15 lines at the end of the harness (group + whitelist filter + fail-on-unexpected).
...
- **PROJECT.md row flip** — Key Decisions row update wording mirrors Phase 9 convention with v1.2 tag and inline path to evidence:
  ...
  - Miss: `⚠️ Revisit (v1.2 rerun — N/5 routes under threshold; follow-up tied to next perf-budget change; see .planning/closure/UIDN-02-mobile-evidence.md § v1.2 Rerun)` (D-20: "under threshold" — generic phrasing covers Perf, A11y, BP, and SEO threshold misses since audit-mobile.sh:54-58 already fails any category below its respective threshold)
...
- **D-05:** **Hard-fail sha256 uniqueness check across all 42 PNGs (with D-19 per-width whitelist).** After the screenshot run, group PNGs by sha256; a collision group passes ONLY if its filenames are exactly the per-width whitelist pair `{bp-{w}-home.png, bp-{w}-admin.png}` (D-19 — intentional collision because unauth `/admin` redirects to `/`). All other collision groups log the offending paths and exit non-zero. Catches the Phase 9 defect signature precisely (`54d4f4916284…` collision across 4 PNGs in v1.1). Free — MANIFEST already records sha256. Brings `audit-screenshots.mjs` to fail-loud parity with `audit-mobile.sh`.
...
- **D-27:** **Plan 13-02 Task 1 Lighthouse log capture must use the `mktemp` + `bash -c` + `cp` pattern** — not naive `tee` into the artifacts directory. The naive form (`bash audit-mobile.sh 2>&1 | tee .planning/closure/artifacts/lighthouse/audit-mobile.stdout.log; echo "exit=${PIPESTATUS[0]}" >> ...`) has TWO defects in the actual runtime environment: (1) `audit-mobile.sh` line 28 executes `rm -rf "$ARTIFACTS_DIR"` (where `$ARTIFACTS_DIR=.planning/closure/artifacts/lighthouse`) BEFORE producing reports — teeing into that directory races with the cleanup and the log is destroyed; (2) this environment's executor shell is zsh, where `${PIPESTATUS[0]}` expands to empty string (zsh uses lowercase `pipestatus` and is 1-indexed) — the appended canonical record reads `exit=` instead of `exit=0|1`. Empirically reproduced in cycle-2 review (`13-REVIEWS.md § Cycle 2 — Empirical Verification`). The corrected form: `tmp_log=$(mktemp); bash -c 'bash .planning/closure/audit-mobile.sh 2>&1 | tee "$1"; exit "${PIPESTATUS[0]}"' _ "$tmp_log"; status=$?; mkdir -p .planning/closure/artifacts/lighthouse; cp "$tmp_log" .planning/closure/artifacts/lighthouse/audit-mobile.stdout.log; echo "exit=$status" >> .planning/closure/artifacts/lighthouse/audit-mobile.stdout.log; rm -f "$tmp_log"`. Why this works: mktemp lives outside the deleted directory (survives the `rm -rf`); `bash -c '... | tee ...; exit "${PIPESTATUS[0]}"'` forces bash semantics so PIPESTATUS works regardless of caller shell; `cp` happens AFTER the script's own `mkdir -p` recreates the directory at audit-mobile.sh:29.
...
- **D-17:** **PROJECT.md Key Decisions row update wording mirrors Phase 9 convention with v1.2 tag and inline path to evidence.** Pass: `✓ Good (v1.2 — Lighthouse Perf 5/5 ≥ 90; see .planning/closure/UIDN-02-mobile-evidence.md § v1.2 Rerun)`. Miss: `⚠️ Revisit (v1.2 rerun — N/5 routes under threshold; follow-up tied to next perf-budget change; see .planning/closure/UIDN-02-mobile-evidence.md § v1.2 Rerun)` (D-20: "under threshold" not "under 90" so the same wording covers any failing category).
...
- **Exact memberUser fixture is `11111111-1111-1111-1111-111111111111`** — confirmed in `e2e/fixtures/test-users.ts:21-24`; per D-23, the harness (`.mjs`) inline-mirrors the UUID + email literal with a source-comment referencing the canonical `.ts` fixture file (runtime `.mjs → .ts` import is rejected by Node's loader; this matches the existing `ADMIN_FIXTURE` declaration at `.planning/closure/audit-screenshots.mjs:53-56`).
